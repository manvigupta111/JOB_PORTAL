...
const jobListing = () => {
  ...
  const clearFilters = () => {
    ...
    return <div>
      <h1 className="gradient-title font-extrabold text-6xl sm:text-7xl text-center">Latest Jobs</h1>
      ...
      <Button onClick={clearFilters} variant="destructive" className="sm:w-1/2">Clear Filters</Button></div>
...

      {
        loadingJobs === false && (
          <div className="mt-8 grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {jobs?.length ? (
              jobs.map((job ,idx) => {
                return <span><JobCard key={job.id} job = {job} savedInit={job?.saved?.length > 0}/></span>
              })
